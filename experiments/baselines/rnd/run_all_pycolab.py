...
if __name__ == '__main__':
  ...
  common_exp_args = [
    ...
    "-ratio_clip 0.1",
    "-normalize_advantage",
    "-normalize_reward", # Let's try if we need tihs
    # "-normalize_extreward",
    "-dual_policy default",
  ...
  for job_idx, (n_tasks, env, run) in enumerate(sweep(itertools.product(envs, runs),
    ...
    for task_idx in range(n_tasks):
      ...
      args.append(f"-log_dir ./results/normalize_reward/ppo/{env[task_idx]}/rnd/ppo_{env[task_idx]}/{exp_id()}/run_{run[task_idx]}")
  ...
  launch("toy_rnd", all_job_args[start_task_id: end_task_id + 1],
