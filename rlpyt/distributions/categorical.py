...
class Categorical(DiscreteMixin, Distribution):
    ...
    def kl(self, old_dist_info, new_dist_info, detach='none'):
    ...
    def likelihood_ratio(self, indexes, old_dist_info, new_dist_info, stopgrad='none'):
        num = select_at_indexes(indexes, new_dist_info.prob)
        den = select_at_indexes(indexes, old_dist_info.prob)
        if stopgrad == 'none':
            return (num + EPS) / (den + EPS)
        elif stopgrad == 'num':
            return (num.clone().detach() + EPS) / (den + EPS)
        elif stopgrad == 'den':
            return (num + EPS) / (den.clone().detach() + EPS)
        elif stopgrad == 'both':
            return (num.clone().detach() + EPS) / (den.clone().detach() + EPS)