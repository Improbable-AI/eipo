...
def generalized_advantage_estimation(reward, value, done, bootstrap_value,
    ...
    nd = nd.type(reward.dtype) if isinstance(nd, torch.Tensor) else nd
    bootstrap_value = bootstrap_value.squeeze(-1) # NOTE: don't know why shape is wrong, this is workaround
    value = value.squeeze(-1)
    # print(advantage.shape, reward.shape, discount, bootstrap_value.shape, nd.shape, value.shape)
